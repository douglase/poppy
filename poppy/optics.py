...
class dispersion_plate(AnalyticOpticalElement):
    """
    ...
    def __init__(self,
                 ...
                 **kwargs):
        AnalyticOpticalElement.__init__(self, name=name,  planetype= planetype,**kwargs)
        self.wavefront_display_hint = 'phase' # preferred display for wavefronts at this plane
        self.d = d
        ...
        if custom_function is None:
            self.n = self.n_sellmeier
        else:
            self.n = custom_function
            
    def n_sellmeier(self,wlengths):
        """
        Sellmeier Equation for the refractive index, n(wavelength),
         of the initialized coefficients or material.
    ...
    def get_opd(self, wave):
        '''
        return optical path difference in meters as a float
        '''
        opd = self.d.to(u.m).value*self.n(wave.wavelength)
        return opd


class refractive_surface(AnalyticOpticalElement):
    """
    ...
    **kwargs : passed to parent class, AnalyticOpticalElement.
    ...
    def __init__(self, x_y_incidence = (0*u.deg,0*u.deg),
                 primary_material = dispersion_plate(),
                 secondary_material = dispersion_plate(material="vacuum"),
                 **kwargs):
        self.x_y_incidence=x_y_incidence
